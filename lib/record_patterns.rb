...
module RecordPatterns
  ...
  def reverse_ipv4_fragment?( value )
    value =~ /\A(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){0,3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\z/
  end
  ...
  def reverse_ipv6_fragment?( value )
    value =~ /\A(?:(?:[a-fA-F0-9])\.){1,31}(?:[a-fA-F0-9])\z/
  end
